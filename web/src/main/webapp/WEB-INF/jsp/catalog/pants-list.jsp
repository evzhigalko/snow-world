...
<div class="container" style="height: 7rem">
    <c:if test="${sessionScope.ROLE eq 'ADMIN'}">
        <a class="btn btn btn-success btn-sm add-new-item" href="<c:url value="/catalog/pants"/>" role="button">Добавить штаны</a>
    </c:if>
</div>
<ul class="list-group">
    <div class="container">
                                ...
                                <div class="col-md-6">
                                    <div class="card-body">
                                        ...
                                        <p class="card-info">Доступен к прокату:
                                            <c:choose>
                                                <c:when test="${pants.availableToRental eq 'true'}">
                                                    <c:out value="${'Да'}"/>
                                                </c:when>
                                                <c:when test="${pants.availableToRental eq 'false'}">
                                                    <c:out value="${'Нет'}"/>
                                                </c:when>
                                            </c:choose></p>
                                        <c:if test="${sessionScope.ROLE eq 'ADMIN'}">
                                            <form action="<c:url value="/catalog/pants/${pants.id}"/>"
                                                  method="post">
    ...
    <ul class="pagination justify-content-center pagination-lg">
        <c:forEach begin="1" end="${requestScope.pagesNumber}" var="i">
            <li class="page-item"><a class="page-link"
                                     href="${pageContext.request.contextPath}/catalog/pants/${i}">${i}</a></li>
        </c:forEach>
    </ul>
</nav>
</body>
